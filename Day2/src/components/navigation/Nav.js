...
class Nav extends React.Component {
	...
	createLinks() {
		const links = this.state.routes.map(node => {
			...
			else this.setState({ subNodes: [] });

			let activeClass = isActive ? " active" : "";

			let classes =
				"component-navigation nav-list nav-listitem" + activeClass;
			return (
				<li className={classes} key={node.title}>
		...
		return (
			<div className="layout-head">
				<div className="component-navigation">
					<ul className="component-navigation nav-list">
						{this.state.links}
					</ul>
				</div>
